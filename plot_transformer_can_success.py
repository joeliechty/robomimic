#!/usr/bin/env python3
"""
Plot success rates for transformer models trained on the can dataset.
...
def collect_transformer_can_data(base_dir, dataset_size, has_divergence, max_seeds=10, save_frequency=20):
    """
    Collect success rates for transformer models on can dataset across multiple seeds.
    ...
    for epoch in epochs:
        for seed in range(max_seeds):
            # Construct filename
            filename = f"transformer_can_{dataset_size}_1000_{save_frequency}_{suffix}_img_epoch{epoch}_seed{seed}_stats.json"
...
def plot_success_rates(eval_data_dir, output_file='transformer_can_success_plot.png'):
    """
    Create plot comparing transformer model success rates on can dataset.
    ...
    for dataset_code, dataset_label, save_freq in dataset_configs:
        ...
        mean_div, std_div = collect_transformer_can_data(eval_data_dir, dataset_code, has_divergence=True, save_frequency=save_freq)
        if mean_div:
        ...
        if save_freq == 20:
            mean_nodiv, std_nodiv = collect_transformer_can_data(eval_data_dir, dataset_code, has_divergence=False, save_frequency=save_freq)
            if mean_nodiv:
    ...
    ax.set_title('Transformer Model Success Rates on Can Dataset', 
...
if __name__ == '__main__':
    ...
    # Create the plot
    output_file = os.path.join(script_dir, 'transformer_can_success_plot.png')
