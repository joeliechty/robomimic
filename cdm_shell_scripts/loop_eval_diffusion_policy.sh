...
if [ -z "$1" ] || [ -z "$2" ] || [ -z "$3" ] || [ -z "$4" ] || [ -z "$5" ]; then
  ...
  echo "Usage: bash loop_eval_diffusion_policy.sh <TASK> <DATASET_PORTION> <TRAINING_EPOCHS> <SAVE_FREQ> <SEED> [START_EPOCH] [END_EPOCH] [EVAL_FREQ] [VIDEO_FLAG]"
  echo "Example: bash loop_eval_diffusion_policy.sh lift full 500 20 0 100 500 40 V"
