...
if [ -z "$1" ] || [ -z "$2" ] || [ -z "$3" ] || [ -z "$4" ] || [ -z "$5" ]; then
  ...
  echo "Usage: bash loop_eval_diffusion_policy.sh <TASK> <DATASET_PORTION> <TRAINING_EPOCHS> <SAVE_FREQ> <SEED> [START_EPOCH] [END_EPOCH] [VIDEO_FLAG]"
  echo "Example: bash loop_eval_diffusion_policy.sh lift full 500 20 0 100 500 V"
...
# Launch Diffusion Policy Baseline Evaluation Loop
docker run -d \
  --name loop_eval_diffusion_base_${TASK}_${DATASET_PORTION}_${TRAINING_EPOCHS}_${SAVE_FREQ}_seed${SEED} \
  --gpus all \
  --net=host \
  -v $(pwd):/app/robomimic \
  -w /app/robomimic \
  robomimic \
  /bin/bash -c "source /opt/conda/etc/profile.d/conda.sh && conda activate robomimic_venv && pip install -e . && python eval_model.py -M diffusion -T ${TASK} -DS ${DATASET_PORTION} -TE ${TRAINING_EPOCHS} -SF ${SAVE_FREQ} -S ${SEED} -SD ${LOOP_ARGS}"
