...
if [ -z "$1" ] || [ -z "$2" ] || [ -z "$3" ] || [ -z "$4" ] || [ -z "$5" ]; then
  ...
  echo "Usage: bash loop_eval_transformer_w_div.sh <TASK> <DATASET_PORTION> <TRAINING_EPOCHS> <SAVE_FREQ> <SEED> [START_EPOCH] [END_EPOCH] [EVAL_FREQ] [VIDEO_FLAG]"
  echo "Example: bash loop_eval_transformer_w_div.sh lift full 500 20 0 100 500 40 V"
...
# Launch Transformer CDM Evaluation Loop
docker run -d \
  --name loop_eval_transformer_cdm_${TASK}_${DATASET_PORTION}_${TRAINING_EPOCHS}_${SAVE_FREQ}_seed${SEED} \
  --gpus all \
  --net=host \
  -v $(pwd):/app/robomimic \
  -w /app/robomimic \
  robomimic \
  /bin/bash -c "source /opt/conda/etc/profile.d/conda.sh && conda activate robomimic_venv && pip install -e . && pip install git+https://github.com/openai/CLIP.git && python eval_rollouts.py -M transformer -CDM -T ${TASK} -DS ${DATASET_PORTION} -TE ${TRAINING_EPOCHS} -SF ${SAVE_FREQ} -S ${SEED} -SD ${IMAGES_FLAG} ${LOOP_ARGS}"

echo "Launched Transformer CDM evaluation loop job for ${TASK} with dataset portion: ${DATASET_PORTION}, training epochs: ${TRAINING_EPOCHS}, save frequency: ${SAVE_FREQ}, seed: ${SEED}"
