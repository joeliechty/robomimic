...
docker run -d \
  ...
  -w /app/robomimic \
  robomimic \
  /bin/bash -c "source /opt/conda/etc/profile.d/conda.sh && conda activate robomimic_venv && pip install -e . && pip install git+https://github.com/openai/CLIP.git && python eval_model.py -M transformer -T ${TASK} -DS ${DATASET_PORTION} -TE ${TRAINING_EPOCHS} -SF ${SAVE_FREQ} -S ${SEED} -SD ${IMAGES_FLAG} ${LOOP_ARGS}"
