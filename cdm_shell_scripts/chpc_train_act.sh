...
TRAIN_ARGS="-D ${DATASET} -DP ${PORTION} -PI ${PORTION_ID} -E ${EPOCHS} -SF ${SAVE_FREQ} -E2E -B ${BATCH_SIZE} -V ${RESUME_FLAG} ${SEED_ARG}"
...
echo "Launching Action Chunking Transformer job for ${DATASET} with portion: ${PORTION}, epochs: ${EPOCHS}, save frequency: ${SAVE_FREQ}"
...
apptainer exec \
  ...
  --pwd /app/robomimic \
  $IMAGE_PATH \
  /bin/bash -c "export NUMBA_CACHE_DIR=/tmp && export PYTHONUSERBASE=/scratch/general/vast/$USER/.local && source /opt/conda/etc/profile.d/conda.sh && conda activate robomimic_venv && pip install --user -e . && python train_act_image.py ${TRAIN_ARGS}"
