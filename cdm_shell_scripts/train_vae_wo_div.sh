...
if [ "$DATASET" = "can" ] || [ "$DATASET" = "square" ]; then
  ...
  docker run -d \
    ...
    -w /app/robomimic \
    robomimic \
    /bin/bash -c "source /opt/conda/etc/profile.d/conda.sh && conda activate robomimic_venv && pip install -e . && python train_divergence_vae_images.py -D ${DATASET} -DP ${PORTION} -PI ${PORTION_ID} -E ${EPOCHS} -SF ${SAVE_FREQ} -I"
else
